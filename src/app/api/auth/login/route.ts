...
import { firebaseFindUserByIdentifier, firebaseMarkLastLogin, firebaseRecordLogin } from "@/lib/firebase-store";
...
export async function POST(request: Request) {
  try {
    ...
    if (isFirebaseBackend()) {
      const user = await firebaseFindUserByIdentifier(input.identifier);
      if (!user) throw new Error("Username/email atau password tidak cocok.");

      const valid = await verifyPassword(input.password, user.passwordHash);
      await firebaseRecordLogin({
        userId: user.id,
        ipAddress: meta.ip,
      ...
      await firebaseMarkLastLogin(user.id);
